(() => {
  ...
  async function renderDashboard() {
    ...
    const recentAuditHtml = (entry) => {
      ...
      const entryAuditor = entry.meta?.auditor?.trim() || "ยังไม่ระบุ";
      return `<article class="recent-audit ${entry.id === audit.id ? "active" : ""}" data-audit-id="${escapeHtml(entry.id)}"><button type="button" class="recent-audit-open" data-recent-action="open" aria-label="เปิดแบบตรวจ ${escapeHtml(entry.meta?.area || "ยังไม่ระบุพื้นที่")} ผู้กรอก ${escapeHtml(entryAuditor)}"><span class="recent-date">${escapeHtml(formatDate(entry.meta?.auditDate))}</span><span><b>${escapeHtml(entry.meta?.area || "ยังไม่ระบุพื้นที่")}</b><small>${escapeHtml(auditTypeLabel(entry.meta?.auditType))} · ${escapeHtml(entry.meta?.site || "ยังไม่ระบุ Site")} · ${escapeHtml(entry.meta?.department || "—")} · ${entrySummary.answered}/${entrySummary.total} ข้อ</small><small class="recent-auditor">ผู้กรอก: ${escapeHtml(entryAuditor)}</small></span><span class="recent-result"><b>${displayPercent(entrySummary.percent)}${entrySummary.percent === null ? "" : "%"}</b><small>${entryFindings} ข้อพบ</small></span></button><div class="recent-audit-actions"><button type="button" class="recent-edit-button" data-recent-action="edit">แก้ไข</button><button type="button" class="recent-delete-button" data-recent-action="delete">ลบ</button></div></article>`;
    };
  ...
  async function showHistory() {
    ...
    els.historyList.innerHTML = audits.length ? audits.map((entry) => {
      const entrySummary = entryStats(entry);
      return `<article class="history-item ${entry.id === audit.id ? "active" : ""}" data-history-id="${entry.id}"><div><h4>${escapeHtml(entry.meta?.area || "ยังไม่ระบุพื้นที่")} · ${escapeHtml(entry.meta?.department || "—")}</h4><p>${escapeHtml(auditTypeLabel(entry.meta?.auditType))} · ${escapeHtml(entry.meta?.site || "ยังไม่ระบุ Site")} · ${formatDate(entry.meta?.auditDate)} · ผู้กรอก: ${escapeHtml(entry.meta?.auditor?.trim() || "ยังไม่ระบุ")} · ${entrySummary.answered}/${entrySummary.total} ข้อ · ${entry.status === "complete" ? "เสร็จสิ้น" : "ฉบับร่าง"}</p></div><div class="history-actions"><button type="button" data-history-open>เปิด</button><button type="button" data-history-delete>ลบ</button></div></article>`;
